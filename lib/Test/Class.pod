...
B<NOTE: This is a development release. Things may change. Documentation is incomplete and inaccurate in places. Be warned.>
...
All the advantages of xUnit testing. You can easily create test fixtures and isolate tests.
...
=head1 2) Setup and teardown methods
...
The above will be run before and after every test method.


=head2 3) Startup and shutdown methods

Startup and shutdown methods are like setup and teardown methods for the whole test suite. All the startup methods are run once when you start running a test class. All the shutdown methods are run once just before a test class stops running.
...
Methods of each type are run in ASCII order.
...
Just like test methods, setup and teardown methods are run in alphabetical order before/after each test method.

Finally startup and shutdown methods are run in alphabetical order before/after when the test suite starts/finishes.
