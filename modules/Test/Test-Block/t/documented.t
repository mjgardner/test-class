...
BEGIN {
	...
	foreach my $module (sort keys %Filename_of_module) {
		...
		if (defined($coverage)) {
			my @uncovered = grep { $_ !~ m/^(TIESCALAR|STORE|FETCH|\(.+)$/s} $pc->uncovered;
			$Test->ok(1, "$module->$_ documented") foreach sort $pc->covered;
			$Test->ok(0, "$module->$_ documented") foreach sort @uncovered;
			$Test->diag("$module undocumented: ". join(', ', @uncovered)) if @uncovered;
		} else {
